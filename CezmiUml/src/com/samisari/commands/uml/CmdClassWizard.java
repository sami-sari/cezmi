...
public class CmdClassWizard extends JDialog {
	...
	private void loadMethods() {
		...
		while (model.getRowCount() > 0)
			model.removeRow(0);
		if (command.getMethods() != null)
			for (CmdClassMethod m : command.getMethods()) {
				Object[] rowData = new Object[] { (m.is_private() ? "private" : (m.is_public() ? "public" : (m.is_protected() ? "protected" : ""))), m.is_abstract(), m.is_static(),
						m.is_final(), m.is_synchronized(), m.is_volatile(), m.is_transient(), m.getReturnType(), m.getName(), "" };
				model.addRow(rowData);
			}
	}
			...
			for (TypeDeclaration type : types) {
				...
				for (BodyDeclaration member : members) {
					if (member instanceof ConstructorDeclaration) {
						ConstructorDeclaration constrtr = ((ConstructorDeclaration) member);
						int md = constrtr.getModifiers();
						String name = constrtr.getName();
						String typ = "";
	...
	private void save() {
		...
		getCommand().setMethods(new ArrayList<>());
		getCommand().setFields(new ArrayList<>());
		...
		for (int row = 0; row < rowCount; row++) {
			CmdClassField field = new CmdClassField();
			...
			if (isFinal) {
				sb.append("final");
				sb.append(" ");
				field.set_final(true);
			}
			if (isSynchronised) {
				sb.append("synchronized");
				sb.append(" ");
				field.set_synchronized(true);
			}
			if (isVolatile) {
				sb.append("volatile");
				sb.append(" ");
				field.set_volatile(true);
			}
			if (isTransient) {
				sb.append("transient");
				sb.append(" ");
				field.set_transient(true);
			}
			sb.append(dataType);
			sb.append(" ");
			...
			sb.append(";\n");
			getCommand().getFields().add(field);
		}

		rowCount = methodsTable.getRowCount();
		sb = new StringBuilder();
		for (int row = 0; row < rowCount; row++) {
			CmdClassMethod mthd = new CmdClassMethod();
			...
			mthd.setName(name);
			mthd.setReturnType(dataType);
			mthd.set_abstract(isAbstract);
			mthd.set_static(isStatic);
			mthd.set_final(isFinal);
			mthd.set_synchronized(isSynchronised);
			mthd.set_volatile(isVolatile);
			mthd.set_transient(isTransient);
			mthd.setArguments(new ArrayList<>());
			String[] args = parameters.split(",");
			if (parameters.trim().length() > 0) {
				for (String ar : args) {
					CmdClassMethodArgument arg = new CmdClassMethodArgument();
					if (ar.startsWith("final")) {
						arg.set_final(true);
						ar = ar.substring(0, ar.indexOf("final") + "final".length());
					}
					arg.setType(ar.substring(0, ar.indexOf(" ")));
					arg.setName(ar.substring(ar.indexOf(" ")));
					mthd.getArguments().add(arg);
				}
			}
			if (visibility != null && visibility.length() > 0) {
				sb.append(visibility);
				sb.append(" ");
				if (visibility.equals("private"))
					mthd.set_private(true);
				else if (visibility.equals("protected"))
					mthd.set_protected(true);
				else if (visibility.equals("public"))
					mthd.set_public(true);
			}
			if (isAbstract) {
				sb.append("abstract");
				sb.append(" ");
			...
			sb.append(");\n");
			getCommand().getMethods().add(mthd);
		}
	}
